...
const layout = () => {
  circleRefs.value.forEach(circle => {
    ...
    gsap.set(circle as HTMLElement, {
      xPercent: -50,
      scale: 0,
    ...
    tl.to(circle as HTMLElement, { scale: 1.2, xPercent: -50, duration: 2, ease: props.ease, overwrite: 'auto' }, 0);
...
onMounted(() => {
  ...
  const menu = mobileMenuRef.value as HTMLElement | null;
  if (menu) {
    gsap.set(menu, { visibility: 'hidden', opacity: 0, scaleY: 1, y: 0 });
  }

  if (props.initialLoadAnimation) {
    const logo = logoRef.value as HTMLElement | null;
    const navItems = navItemsRef.value as HTMLElement | null;
...
const handleLogoEnter = () => {
  ...
  gsap.set(img as HTMLElement, { rotate: 0 });
  logoTweenRef.value = gsap.to(img as HTMLElement, {
    rotate: 360,
    duration: 0.2,
    ease: props.ease,
    overwrite: 'auto'
...
const toggleMobileMenu = () => {
  ...
  const hamburger = hamburgerRef.value as HTMLElement | null;
  const menu = mobileMenuRef.value as HTMLElement | null;

  if (hamburger) {
    const lines = hamburger.querySelectorAll('.hamburger-line');
    if (newState) {
      gsap.to(lines[0] as HTMLElement, { rotation: 45, y: 3, duration: 0.3, ease: props.ease });
      gsap.to(lines[1] as HTMLElement, { rotation: -45, y: -3, duration: 0.3, ease: props.ease });
    } else {
      gsap.to(lines[0] as HTMLElement, { rotation: 0, y: 0, duration: 0.3, ease: props.ease });
      gsap.to(lines[1] as HTMLElement, { rotation: 0, y: 0, duration: 0.3, ease: props.ease });
    }
  }
...
const setCircleRef = (el: any, index: number) => {
  if (circleRefs.value.length > index) {
    circleRefs.value[index] = el as HTMLSpanElement | null;
  }
};

const handlePillEnter = (e: MouseEvent) => {
  const target = e.currentTarget as HTMLAnchorElement;
  target.style.background = 'var(--base)';
  target.style.color = 'var(--hover-text, #fff)';
};

const handlePillLeave = (e: MouseEvent) => {
  const target = e.currentTarget as HTMLAnchorElement;
